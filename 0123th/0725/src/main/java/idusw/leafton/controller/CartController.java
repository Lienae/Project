...
public class CartController {
    ...
    // 바로 구매시
    @PostMapping(value = "/cart/one/{memberId}/{productId}")
    public String addOneCartItem(@PathVariable("memberId") Long memberId, @PathVariable("productId") Long productId, int count) {

        MemberDTO member = memberService.getMemberById(memberId);
        ProductDTO product = productService.getProductById(productId);

        Long cartItemId = cartService.addOneCart(member, product, count);


        return "redirect:/pay/buy/one/{memberId}/" + cartItemId;
    }
